      ...
      for (size_t readno = 0; readno < readSet.size(); readno++)
      {
        ...
        for (int32_t i = 0; i < readLength; i++)
        {
          ...
          for (int32_t j = 0; j < graph.numVertices; j++)
          {
            ...
            int32_t matchScore = ((curChar == 'N') || (readSet[readno][i] == 'N') || (curChar == readSet[readno][i])) ? parameters.match : -1 * parameters.mismatch;
...
#ifdef PASGAL_SCORE_ONLY
      // Paper score-only benchmarks: stop after forward DP (skip reverse DP + cigar).
      outputBestScoreVector = bestScoreVector_P1;
      return;
#endif
    }
